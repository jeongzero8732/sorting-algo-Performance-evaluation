...
void quick_year(DataSet* dataSet, int start, int end, DataSet* tmp)
{
	...
	/*while (p < r)
	{
		...
		while (dataSet[r].year >= pivot && p < r)
			r--;
		if (p != r)
			memcpy(&dataSet[p], &dataSet[r], sizeof(DataSet));

		while (pivot >= dataSet[p].year && p < r)
			p++;
		if (p != r)
		{
			memcpy(&dataSet[r], &dataSet[p], sizeof(DataSet));
			r--;
		}
	}

	dataSet[p].year = pivot;
	pivot = p;
	p = l_hold;
	r = r_hold;

	if (p < pivot)
		quick_year(dataSet, p, pivot - 1);
	if (r > pivot)
		quick_year(dataSet, pivot + 1, r);*/
...
void insert_sorting_title(DataSet* dataSet, int* lineCount)
{
	...
	for (j = 1; j < *lineCount; j++)
	{
		...
		{
			/*   if ((strcmp(dataSet[i].title, key->title)) == 0)
			   {
				  if (key->title_id < dataSet[i].title_id)
				  {
					 memcpy(key2, &dataSet[i], sizeof(DataSet));
					 memcpy(&dataSet[i],key,sizeof(DataSet));
					 memcpy(key,key2,sizeof(DataSet));
				  }
			   }*/
...
{
	DataSet* tmp = (DataSet*)malloc(sizeof(DataSet) * (*lineCount));
	//DataSet* tmp = (DataSet*)malloc(sizeof(DataSet));
